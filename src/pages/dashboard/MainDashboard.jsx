...
const MainDashboard = () => {
  ...
  const loadSession = useSelector((state) => state.session.loading);
  const loadBooking = useSelector((state) => state.booking.loading);
  ...
  return (
    <section className='h-full w-full mt-8 mb-8'>
            ...
            <table className='w-full rounded-lg text-[#373737] bg-white'>
              ...
              <tbody>
                {datasSession.map((session, index) => (
                  ...
                  >
                    <th className='font-normal py-4 w-[200px]'>
                      {session.date}
                    </th>
